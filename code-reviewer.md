...
## 범위 규율 (과잉설계 · 범위 이탈)
동작하는 코드여도 **요청한 것만 했는지**를 따로 본다. 이 렌즈는 스택과 무관하게 모든 리뷰에 적용한다.
- **요청에 없는 것**: 쓰이지 않을 확장점·설정·유연성, 호출자가 하나뿐인 추상화(인터페이스·팩토리·제네릭 래퍼·설정 클래스), 일어날 수 없는 경우의 예외 처리, 미리 만들어 둔 "나중에 쓸" 훅.
- **분량 대비 일**: 같은 일을 절반 분량으로 할 수 있으면 지적하고 축약안을 제시한다. 기준은 "시니어 개발자가 과하다고 할 코드인가".
- **범위 이탈**: 변경된 줄이 요청으로 역추적되지 않으면 지적한다 — 무관한 리팩터·주석 정리·포맷 변경·인접 코드 "개선", 기존 관용구를 두고 새로 들여온 스타일.
- **고아 정리의 범위**: 이번 변경 때문에 쓰이지 않게 된 import·변수·함수가 남았으면 지적한다. 반대로 **원래부터 있던 죽은 코드를 함께 지웠으면** 그것도 범위 이탈로 지적한다(지울지는 사용자가 정한다).

심각도는 기본 **Should fix**로 낸다. 다만 요청 범위 밖에서 동작이 바뀌었으면 Must fix다.
...
**Top 3는 재인용이 아니라 색인이다.** 항목 제목과 `파일경로:줄번호`, 그리고 "왜 먼저인가"를 한 줄로만 적는다. 근거·영향·수정 방향을 본문에서 그대로 다시 쓰지 않는다 — 같은 내용을 두 번 읽게 만들지 않는 것이 목적이지, 발견을 줄이라는 뜻이 아니다. 줄일 것은 중복된 설명이지 증거·위치·불확실성 표기가 아니다.
