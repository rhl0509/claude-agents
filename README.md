...
- 현재 버전: `test-runner` **v1.7**, `code-reviewer`·`security-reviewer`·`db-optimizer` **v1.6**, `data-modeler`·`devops-reviewer`·`ui-ux-reviewer` **v1.4**, `system-architect`·`design-system-architect`·`api-doc-writer` **v1.3**, `perf-auditor`·`test-strategy` **v1.2**, `migration-reviewer` **v1.1** — 상세 이력은 [CHANGELOG.md](CHANGELOG.md)
...
| 2 | `security-reviewer` | `/sec` | 품질 | 1.6 | opus | 보안 취약점(OWASP) 점검 | Read, Grep, Glob, WebSearch, WebFetch |
