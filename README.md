...
| 67 | `identity-access-architect` | `/autharch` `/인증설계` | 1.1 | opus | **인증·인가·세션 구조 설계**(플로우 검증·세션 결정표·패스키·SSO/SCIM·테넌트 격리) | Read, Grep, Glob, Context7 |
...
| 77 | `motion-reviewer` | `/motion` `/모션` | 1.1 | opus | **모션·애니메이션 품질**(빈도 게이트·이징·300ms 예산·원점·인터럽트·GPU 속성·제스처 물리) + 기회 탐색(기각 후보 필수) | Read, Grep, Glob |
| 76 | `slide-deck-reviewer` | `/deck` `/ppt` `/발표` `/슬라이드` `/프레젠테이션` `/프리젠테이션` | 1.2 | opus | **발표 슬라이드 덱**(1440×810 고정 캔버스) 규약 점검 — 배열·페이지번호 동기화·캔버스 이탈·투사 가독성·에셋 대조·템플릿 잔재 | Read, Grep, Glob |
...
| # | 에이전트 | 슬래시 | 버전 | 모델 | 역할 | 도구 |
|---|---|---|---|---|---|---|
| 48 | `c-code-reviewer` | `/creview` | 1.1 | opus | **C 코드 리뷰**(메모리 안전·UB·정수 변환·에러경로 누수·포맷 취약점) | Read, Grep, Glob, Bash |
...
| 51 | `dotnet-code-reviewer` | `/dnreview` | 1.1 | opus | **비-Unity C#/.NET 리뷰**(async 데드락·IDisposable·지연실행·DI 수명·EF Core) | Read, Grep, Glob, Bash |
...
| 55 | `java-code-reviewer` | `/jreview` | 1.1 | opus | **Java(JVM) 코드 리뷰**(NPE·Optional 오용·예외 정책·자원 누수·동시성·equals/hashCode·오토박싱 ==·Stream 부작용 · 전담 perf 없음) | Read, Grep, Glob, Bash |
...
| 57 | `swift-code-reviewer` | `/swreview` | 1.1 | opus | **Swift 코드 리뷰**(강제 언랩 크래시·ARC retain cycle·값/참조·에러 삼킴·동시성 actor/@MainActor/Sendable·열거 망라·Codable · 전담 perf 없음) | Read, Grep, Glob, Bash |
...
| 70 | `image-prompt-engineer` | `/imgprompt` `/이미지프롬프트` | 1.2 | opus | **AI 이미지 프롬프트 5계층 설계** + 반복 대상의 일관성 시트 + 권리 경계(실존 인물·브랜드 거부) | Read, Grep, Glob |
...
- **점검**: ① 슬라이드 파일 ↔ 뷰어 `slides` 배열 ↔ import ↔ **하드코딩 페이지 번호** 동기화(고아 슬라이드·번호 건너뜀·중복 — 빌드가 통과해 발표 당일에야 드러나는 결함) ② 고정 캔버스 이탈(`.slide` 최상위 누락·1440×810 밖 좌표·스케일과 충돌하는 vh/%/미디어쿼리·잘림 위험) + **투사 가독성 최소 폰트**(본문 14px·카드 제목 16px) ③ 조작 UI 계약(`data-nav` 누락 시 클릭하면 슬라이드가 넘어감·`stopPropagation`·시스템 `alert` 금지·`use client` 과부족) ④ 에셋 참조 대 실제 파일 **전수 대조**(결손·미사용)와 디렉터리 혼동·비ASCII 파일명·접두사 규칙 ⑤ **템플릿 잔재**(package.json name·포트 충돌·metadata·PPTX 출력 파일명·로고·이전 프로젝트 팀 표기) ⑥ 빌드·의존성(`turbopack: {}` 누락, 동적 import라 빌드는 통과하고 **실행 시점에 죽는** 패키지 미선언) ⑦ **발표 장비 이식성(1.2)** — 웹폰트 CDN 단일 의존(오프라인 시 폴백 폭 차이 → 줄 추가 → 잘림)·이모지 아이콘의 장비별 글리프 차이(→인라인 SVG). 같은 판에서 **슬라이드 간 정렬선 일관성**(래퍼별 좌우 패딩 / `.content`↔`.contentTop` 혼용 — 연속 재생에서만 드러난다)과 **아트 위 텍스트 스크림 부재**도 본다
- **심각도**: 코드 냄새가 아니라 **발표 중 관객에게 보이는 결과** 기준 — 치명(슬라이드 누락·깨진 이미지·다른 프로젝트 이름 노출·클릭 오작동) / 주의(안 읽힘·잘림·번호 어긋남) / 제안(규약 이탈이나 이번 발표엔 영향 없음)
- **다루지 않는 것(내용, 1.1)**: 수치의 사실성·상호 정합(→`fact-checker`), 발표 대본과 슬라이드 내용 일치(→작성자), 챕터 구성·목차 순서·장 분할(→`proposal-strategist`·`curriculum-designer`), 문구 설득력·분량(→`copy-reviewer`)은 **프로젝트마다 달라 판정하지 않는다**. **경계선 판별법** — 규약 문서에 적을 수 있으면 틀, 프로젝트가 바뀌면 답이 바뀌면 내용("페이지 번호가 배열과 어긋난다"=틀 / "Chapter 8 뱃지가 목차와 다르다"=내용)
- **한계**: 실제 넘침·투사 가독성·PPTX 충실도는 정적으로 단정하지 않고 "확인 필요"와 확인 방법으로 분리. **넘침·폰트 하한은 실측 절차로 닫는다(1.2)** — `npm run dev`로 띄워 전 장을 순회하며 `getBoundingClientRect()`를 캔버스 좌표로 환산(`k = rect.width/1440`)해 1440×810 이탈을 모으고, 텍스트 노드를 **직접** 가진 요소의 `fontSize`만 세어 14px 미만을 잡는다(에이전트는 실행 권한이 없으므로 메인 세션이 돌릴 절차로 제시)
- **출력**: 요약(정본 경로·건수) → Top 3 → 치명/주의/제안(`파일:줄` 고정) → 확인 필요
- **구분**: 발표에 **무엇을 담을지**(서사·설득)는 `proposal-strategist`(제안·입찰)·`curriculum-designer`(강의)·`content-repurposer`(멀티포맷), 문장 품질은 `copy-reviewer`, 보이스는 `brand-voice-guardian`, 수치 검증은 `fact-checker`. 일반 React/Next 품질은 `code-reviewer`, **일반 웹 화면**은 `ui-ux-reviewer`, 토큰 설계는 `design-system-architect`, 번들·렌더 성능은 `perf-auditor`, 이미지 생성 프롬프트는 `image-prompt-engineer`, 게임 HUD는 `game-ui-reviewer`
</details>
...
- **표 진입 게이트(1.1)**: 서술만으로는 안 지켜져 **기계적 게이트 3종**으로 못박음 — ⒜ 마감일을 원문에서 **직접 확인한 공고만** 관리표에 올리고 미확인 건은 우선순위·적합도 없는 별도 섹션(`확인 못 한 후보`)으로 내린다("확인 필요"는 셀 값이 아니라 **표 밖으로 나가는 사유**) ⒝ **접수 시작 60일 경과** 공고는 마감일을 몰라도 마감 간주 제외(상시채용 명시 확인 시 예외) ⒞ 공고명·시즌·업계 관행 추론("하반기니까 열려 있겠지")은 표 진입 근거가 **아니다** — 후보가 모자라면 추론으로 채우지 말고 빈칸으로 두고 사유를 적는다
- **수집 실패 보고(1.1)**: 탐색 소스 **과반이 차단**(403·로그인·JS 렌더 실패)돼 원문을 못 읽으면 표를 짜내지 않고 **"오늘 수집 실패"**를 결론으로 낸다. 스니펫으로만 본 회사·직무는 후보가 아니라 **단서**로 표시. 차단은 실행 환경(로컬/클라우드·IP·UA)에 따라 달라지는 그날의 사실이지 그 사이트의 항구적 성질이 아니므로 다음 실행에서 재시도
- **선별**: 대기업·대형 계열사 우선(부족하면 이유 먼저 + 적합도 높은 중견/중소/스타트업 보조후보), 이미 지원·마감 제외, 정규직 우선, 요구사항 **60~70% 매칭이면 후보**, 후순위 신호(석사·논문·계약직 단독 등) 반영
- **출력**: 소스 접근 요약 → 12열 지원 관리표(마감일 확인 건만, 목표 10건이되 **모자라면 적은 채로**) → `확인 못 한 후보` 표 → 공고별 적합 이유 → 앞세울 포트폴리오 프로젝트 → "오늘 바로 지원할 순서"(1번부터) + 링크
...
5계층(피사체·환경·**조명**·기술·스타일)으로 쌓는다. 조명이 결과를 가장 크게 좌우한다. 모호한 일상어를 사진 용어로 치환하고("배경 흐리게" → `shallow depth of field, f/1.8, creamy bokeh`), 조명 방향과 그림자 묘사의 모순·물리적으로 불가능한 조합을 검문한다. 장르별 슬롯 패턴(인물·제품·풍경·패션)과 플랫폼별 문법, 필름 에뮬레이션 어휘를 제공하고 네거티브 프롬프트와 변주 2~3개를 함께 낸다. **권리·윤리 경계는 신규 집필**(원본이 상업 용도를 전제하면서 이 층이 공백이었다): 실존 인물 얼굴·브랜드 로고 생성 거부, 생존 작가 화풍 요청에는 리스크를 알린 뒤 **기법으로 분해한 대안**을 함께 제시. 1.2에서 **일관성 레이어**를 얹었다 — 같은 대상이 2회 이상 반복되면(단발엔 과잉) 5계층 위에 층을 하나 더 올려 **레퍼런스 시트**로 고정한다. 시트 설계는 패널 배열이 아니라 **축(axis) 설계**다(무엇이 변하고 무엇이 안 변하는가). 대상 유형별 변량 축 표 + 필수 구성 4가지(전 패널 불변 고정문·프로필 텍스트 박스·hex 팔레트·시트 시각 규격의 브랜드 치환) + 접속 규칙(**조명·기술은 패널별이 아니라 시트 전체에 한 번만**, 안 그러면 고정하려던 것이 도로 풀린다) + 2단계 워크플로(시트 생성 → 시트를 입력으로 재첨부)와 플랫폼 네이티브 수단(`--cref`/`--sref`·IP-Adapter·LoRA)의 선택 기준. **인물 시트는 얼굴을 여러 각도로 증식시켜 단발보다 초상권 리스크가 크므로** 본인·동의받은 모델일 때만 진행하고 아니면 가상 인물을 먼저 생성하는 경로를 제시한다.
→ UI 시각 설계는 `design-system-architect`, 화면 점검은 `ui-ux-reviewer`, 썸네일 **컨셉 전략**과 **영상 콘티 기획**은 `video-optimizer`·`content-repurposer`(이쪽은 그 콘티를 시트 이미지로 뽑는 층).
...
**74. aws-reviewer (`/aws`, `/인프라`)** — 🚀 운영 · opus · effort high
**IaC로 선언된 AWS 리소스의 보안·안정성·비용 자세**를 본다: IAM 최소권한(`Action: "*"`·과대 `AssumeRole` Principal·관리형 정책 과대 부여), 공개 노출(퍼블릭 S3/ACL·`0.0.0.0/0` 인바운드·퍼블릭 서브넷의 RDS), 저장 암호화(CMK 대 기본키·회전), 네트워크 격리, 서비스 구성(ECS 태스크 롤·Lambda 타임아웃/동시성·RDS 멀티AZ/백업/삭제보호), Terraform state 백엔드(잠금·버전·암호화·state 속 평문 시크릿), 비용·태그, 그리고 **파괴적 변경**(RDS·EBS replace = 데이터 상실을 별도 경고). devops-reviewer와의 경계는 주제가 아니라 **산출물**로 긋는다 — 같은 OIDC라도 `.tf`/CDK/CFN의 IAM 역할·신뢰정책은 aws-reviewer, Actions 워크플로 YAML은 devops-reviewer. 읽기 전용 — AWS 명령을 직접 실행하지 않으며 실제 배포는 `aws-deploy` 스킬(메인 세션)이 맡는다.
→ 컨테이너·CI/CD·시크릿 파이프라인은 `devops-reviewer`, 웹 앱 취약점은 `security-reviewer`, 설계 단계 위협 모델링은 `threat-modeler`, 배포 실행은 `aws-deploy` 스킬.
...
| ui-ux-reviewer → slide-deck-reviewer | 웹 화면 → 발표 덱 | 고정 캔버스·스케일·투사 전제는 발표 덱으로(발표 덱은 일반 웹 화면을 역참조 안 함) |
...
### 3) 전역 등록 (Windows)
저장소의 74개 에이전트 `.md`를 전역 폴더로 복사합니다. 동봉된 스크립트를 쓰면 편합니다.
```powershell
...
| `/aws` | aws-reviewer | IaC 경로(선택) |
| `/aws-deploy` | (스킬) 메인 세션 AWS 배포 워크플로 | 대상 환경 |
| `/지도` | (외부 스킬 graphify) 코드 지도 생성·갱신·조회 | 질문 또는 경로(선택) |
| `/지도대조` | (메인 세션 절차) 크로스커팅 관심사 전수 대조 | 대조할 함수/관심사 |

### 한글 별칭 (1.65)

Claude Code는 슬래시 명령 이름에 **한글(비ASCII)을 허용한다**(1.65에서 `/디버그`로 검증). 자주 쓰는 49종에 한글 별칭 커맨드를 함께 배포한다 — 에이전트는 그대로고 커맨드 파일만 하나 더 있는 구조라, 별칭을 지워도 영어 명령은 그대로 동작한다.
...
> **한글 이름이 정본인 커맨드 (1.114)**: `/지도`·`/지도대조`는 영어 커맨드의 별칭이 아니라 한글 이름 자체가 정본이다. `/지도`의 영어 대응은 이 레포가 아니라 graphify가 자체 설치하는 `/graphify` 스킬이고, `/지도대조`는 영어 쌍이 없다(필요해지면 신설).
...
- 이 README의 [에이전트 표](#에이전트-74종) 버전 칸도 버전업 시 함께 갱신됩니다.
