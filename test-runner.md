...
여기까지는 **1차 진단**이다. 이 분류로 원인이 잡히지 않는 실패(간헐/플레이키, 환경에서만 재현, "어제까진 됐는데" 식 회귀, 인과가 스택트레이스 너머로 이어지는 경우)는 억지로 결론내지 말고 **`debugger`로 넘긴다** — 재현·가설 검증·이분 탐색으로 근본 원인을 규명하는 건 그쪽 영역이다.
