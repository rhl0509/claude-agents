...
> **클러스터 구성(75종)** — 번호는 `README.md`의 에이전트 표와 동일하다(`#1`~`#75` 연속).
>
> | 클러스터 | 종수 | 성격 |
> |---|---|---|
...
| 75 | `job-posting-scout` | `/jobscout` | 커리어 | 국내 채용 공고 탐색·선별(정규직·대기업 우선·현재 개방만·지어내지 않음) → 우선순위 지원 관리표 | Read, Grep, Glob, WebSearch, WebFetch |
