...
> 에이전트를 추가·삭제·개명하면 `registry.json`을 고치고 `build-registry.ps1`로 PM 라우팅 블록을 재생성한다(명단은 한 벌만 유지한다). 그다음 `sync.ps1` 전에 `lint-agents.ps1`(`/lint`)로 규범 검사를 돌린다 — 프론트매터·읽기 전용 계약·경계 위임절·끊어진 위임 링크·라우팅 고아를 기계가 잡는다.
...
| 76 | `slide-deck-reviewer` | `/deck` `/ppt` | 디자인 | 발표 슬라이드 덱(1440×810 고정 캔버스) 규약 점검 — 배열·번호 동기화·캔버스 이탈·투사 가독성·에셋 대조·템플릿 잔재·장비 이식성 | Read, Grep, Glob |
...
| 70 | `image-prompt-engineer` | `/imgprompt` | 콘텐츠 | AI 이미지 생성 프롬프트 5계층 설계 + 반복 대상의 일관성 시트 + 권리 경계(실존 인물·브랜드 거부) | Read, Grep, Glob |
...
| 75 | `job-posting-scout` | `/jobscout` | 커리어 | 국내 채용 공고 탐색·선별(정규직·대기업 우선·**마감일 확인 건만 표에**·60일 경과 제외·소스 과반 차단 시 수집 실패 보고) → 우선순위 지원 관리표 | Read, Grep, Glob, WebSearch, WebFetch |
...
Next.js + FastAPI 코드의 품질·가독성·버그 가능성 리뷰. `git diff`로 변경분을 파악해 그 범위에 집중(커밋/PR 전 셀프 리뷰). 백엔드(Pydantic·async·DB 세션·예외·계층 분리), 프론트(서버/클라 경계·페칭·useEffect·타입). Next.js 15/16이면 Server Actions 보안·`use cache` 오캐시·React Compiler 중복 수동 메모도 점검(버전 불명확하면 "확인 필요"). **범위 규율**(1.17): 동작하는 코드여도 요청한 것만 했는지를 따로 본다 — 요청에 없는 확장점·1회용 추상화·불가능한 예외 처리, 변경 줄의 요청 역추적(무관한 리팩터·주석 정리·인접 코드 "개선"), 고아 정리의 범위(내 변경이 만든 것만 제거, 기존 죽은 코드 삭제도 이탈). 스택 무관 적용. 출력: 요약 → Must fix → Should fix → Nit. **Top 3는 색인**(제목·`파일:줄`·왜 먼저인가 한 줄)이지 본문 재인용이 아니다(1.18).
...
**75. job-posting-scout (`/jobscout`, `/채용공고`)** — 커리어 (탐색·큐레이션)
국내 채용 사이트(원티드·점핏·사람인·잡코리아·링크드인·리멤버·랠릿·게임잡)와 기업 공식 채용 페이지에서 사용자 프로필·전략에 맞는 **현재 지원 가능한** 공고를 탐색·선별해 우선순위 지원 관리표(12열)로 정리. 대기업·대형 계열사 우선(부족하면 이유 먼저 + 적합도 높은 중견/중소/스타트업 보조후보), 이미 지원·마감 제외, 정규직 우선, 요구사항 **60~70% 매칭이면 후보**, 후순위 신호(석사·논문·계약직 단독 등) 반영. **정직성이 최우선**: 기업명·공고·링크·마감일을 지어내지 않고 확인 가능한 것만 내며, 로그인·차단으로 못 본 소스는 "확인 불가"로 표시하고 직접 확인 URL 제공(추측 금지), 확인 시점 명시 + 지원 직전 재확인 안내. **1.1에서 정직성을 서술이 아니라 기계적 게이트로 못박았다**(원문 미확인 공고가 "확인 필요" 딱지를 단 채 1순위에 오른 실패를 겪고 신설): ⒜ **마감일을 원문에서 직접 확인한 공고만** 표에 올리고 미확인 건은 우선순위·적합도 없는 `확인 못 한 후보` 섹션으로 분리 ⒝ **접수 시작 60일 경과**는 마감 간주 제외(상시채용 명시 시 예외) ⒞ 공고명·시즌 추론은 표 진입 근거가 아니며 후보가 모자라면 빈칸+사유 ⒟ **소스 과반이 차단된 날은 표를 만들지 않고 "수집 실패"를 보고**(스니펫으로만 본 것은 후보가 아니라 단서). 출력: 소스 접근 요약 → 지원 관리표(마감일 확인 건만, 모자라면 적은 채로) → `확인 못 한 후보` → 공고별 적합 이유 → 앞세울 포트폴리오 → "오늘 바로 지원할 순서". 입력(직무·기술·도메인·경력·지역·지원완료목록)이 없으면 되묻는다.
→ 고른 공고용 자소서는 `cover-letter-tailor`(텍스트)·`/cover`(파일 저장 메인 세션), 제안서는 `proposal-strategist`, 사실검증은 `fact-checker`·`truth-checker`.
...
### 🆕 1.98 신설 — `_drafts` 편입 1종

**74. aws-reviewer (`/aws`, `/인프라`)** — 🚀 운영 · opus · effort high
**IaC로 선언된 AWS 리소스의 보안·안정성·비용 자세**를 본다: IAM 최소권한(`Action: "*"`·과대 `AssumeRole` Principal·관리형 정책 과대 부여), 공개 노출(퍼블릭 S3/ACL·`0.0.0.0/0` 인바운드·퍼블릭 서브넷 RDS), 저장 암호화(CMK 대 기본키·회전), 네트워크 격리, 서비스 구성(ECS 태스크 롤·Lambda 타임아웃/동시성·RDS 멀티AZ/백업/삭제보호), Terraform state 백엔드(잠금·버전·암호화·평문 시크릿), 비용·태그, **파괴적 변경**(RDS·EBS replace = 데이터 상실 별도 경고). devops-reviewer와의 경계는 **산출물**로 긋는다 — `.tf`/CDK/CFN의 IAM 역할·신뢰정책은 aws-reviewer, Actions 워크플로 YAML은 devops-reviewer. 읽기 전용 — AWS 명령을 직접 실행하지 않으며 배포는 `aws-deploy` 스킬이 맡는다.
→ 컨테이너·CI/CD는 `devops-reviewer`, 웹 앱 취약점은 `security-reviewer`, 위협 모델링은 `threat-modeler`, 배포 실행은 `aws-deploy` 스킬.
