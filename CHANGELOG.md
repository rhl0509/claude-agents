...
## 1.105 (2026-07-22) — 신설: motion-reviewer(#77) + 프리로드 스킬 motion-reference → 77종

모션은 이 라이브러리의 **실측된 공백**이었다. `ui-ux-reviewer`는 애니메이션을 접근성 한 줄(`prefers-reduced-motion` 대응 유무)과 심미성 보조의 "모션" 한 줄로만 다뤘고, `design-system-architect`의 토큰 세트에는 **이징·듀레이션 토큰이 아예 없었다** — 즉 "이 애니메이션이 굼뜨다"를 판정할 주체도, 그 판정 기준이 될 토큰을 세울 자리도 없었다. [emilkowalski/skills](https://github.com/emilkowalski/skills)(MIT © 2026 Emil Kowalski, 애니메이션 7종 스킬)를 전수 대조해 **이 공백에 해당하는 각도만** 추려 신설했다.

- **[신설] `motion-reviewer` v1.0 (#77)** — 웹(Next.js/React/CSS) 모션·애니메이션 품질 점검. `opus`/`effort: high`/`Read, Grep, Glob`/`color: purple`.
  - **판정을 취향에서 값으로 옮긴 것이 핵심이다.** "느낌이 이상하다"가 아니라 "이 값이 규범에서 벗어났다"로 적게 만든다 — 프리로드 스킬의 값을 **인용하고 근사하지 않는다**는 규칙을 본문에 못박았다.
  - **점검 11축**: 정당성(모션의 목적을 5단어 중 하나로 부를 수 있는가) · **빈도 게이트**(하루 100회 이상·키보드로 시작되는 동작의 애니메이션은 판단이 아니라 **실격**) · 이징(등장·퇴장 `ease-out`, **UI의 `ease-in`은 결함**) · 지속시간 예산(UI 300ms 미만 + 요소별 표) · 물리성·원점(`scale(0)` 금지, 팝오버는 트리거에서 자람 · **모달은 예외**) · **인터럽트 가능성**(빠르게 반복 트리거되는 것에 `@keyframes`는 처음부터 다시 시작 → transition/스프링, 인터럽트 시 목표값이 아니라 **현재 화면값**에서 이어받기) · GPU 속성 · 비대칭 타이밍 · 제스처 물리 · 접근성 · 응집·스태거.
  - **원본 3종(review/improve/find)을 한 에이전트의 두 모드로 통합** — 같은 레퍼런스를 공유하는 두 에이전트를 만들면 라우터가 혼선을 일으키므로, ① 점검(기본) ② 기회 탐색으로 나누고 출력 첫 줄에 모드를 밝히게 했다. 기회 탐색 모드에는 원본의 **절제 규율**을 그대로 가져왔다: 제안 5~7개 상한 + **기각 후보 2~5개 필수**(기각 목록이 없으면 산출물은 위시리스트일 뿐이다).
  - **수정 제안에 순서를 강제**했다 — 지운다 → 줄인다 → 이징 → 원점·물리성 → 인터럽트 → GPU → 비대칭 → 다듬기 → 접근성·응집. "애매하면 지우는 쪽이 대체로 정답"을 명시해, 모션 에이전트가 모션을 늘리는 방향으로만 작동하는 실패를 막았다.
  - **정적 한계 분리** — 코드만으로 "느낌"을 단정하지 않는다. 규범 이탈만 단정하고 나머지는 확인 방법(2~5배 슬로모션·DevTools 애니메이션 패널·제스처는 실기기·다음 날 다시 보기)과 함께 넘긴다. `unity-perf-auditor`·`c-perf-auditor`가 "느리다"를 단정하지 않는 것과 같은 규율이다.
  - 「다루지 않는 것」 표로 제외 범위 6종을 고정하고, 모션 밖 지적은 발견 목록이 아니라 출력 끝 한 줄로만 남기게 했다(`slide-deck-reviewer` 1.103의 선례).
- **[신설] 프리로드 스킬 `motion-reference`** (`skills/motion-reference/`) — `motion-reviewer`·`design-system-architect`에 주입되는 **구체 값 정본**. 빈도 판정표 · 이징 결정 순서와 실제 커브 3종 · 요소별 듀레이션 예산 · 물리성·원점 · 인터럽트(`@starting-style` 포함) · **제스처 물리** · GPU 규칙 · 스태거 30~80ms · 모션 접근성 · 용어 대조표 21항.
  - **출처 처리는 `design-reference`(1.87) 선례를 따랐다** — 방법론과 수치는 인용하되 문장·판정 절차는 저장소 규범으로 다시 쓰고, 출처·라이선스를 스킬 상단에 명시했다.
  - **Apple 층은 모션·제스처만 가져왔다**(스프링 damping/response 값표, 속도 핸드오프, 모멘텀 투영 `project(v) = (v/1000)·d/(1−d)`, 러버밴딩, 1:1 추적, 속도 부호로 커밋 판정). 머티리얼(`backdrop-filter`)·타이포그래피(트래킹/레딩)·8대 원칙은 **의도적으로 제외** — 그 층은 `design-reference`의 관할이고, Apple 특유 미학이 전 프로젝트에 스며드는 것을 막기 위해서다.
- **[역방향 카브아웃] `ui-ux-reviewer` 1.7→1.8** — description에 모션 위임절을 넣고 자신의 범위를 **"`prefers-reduced-motion` 대응 유무까지"**로 좁혔다. 점검 항목 5번(모션 접근성)과 13번(심미성-모션)에도 같은 경계를 달았다. 나머지 항목·출력 형식은 무변경.
- **[확장] `design-system-architect` 1.6→1.7 — 모션 토큰을 색·타이포와 같은 급으로 올렸다.** 「모션 토큰」 절 신설(이징 3종 · 요소 예산에서 유도한 듀레이션 스케일 · 스프링 프리셋 · reduced motion의 토큰 레벨 처리)하고 `motion-reference`를 프리로드에 추가. **DESIGN.md 배치 판단을 명시** — google-labs-code/design.md 프런트매터에 모션은 표준 키가 아니므로, CSS 변수를 정본으로 두고 `## Motion` 산문 섹션으로 근거를 남기게 안내한다(임의 키는 lint 경고 대상이고 `export`로 나가지 않는다).
- **[대칭] `game-feel-reviewer` 1.3→1.4** — 웹 UI 모션은 `motion-reviewer`로 넘기는 절 추가(엔진 손맛과 웹 규범은 판정 기준이 다르다). 점검 항목 무변경.
- **[커맨드] `/motion`·`/모션` 신설** — 81개 슬래시 명령(한글 별칭 55개), 커맨드 파일 총 136개.
- **[레지스트리]** `registry.json`에 `motion-reviewer`(group 화면, note "웹 모션·애니메이션 품질") 추가 → `build-registry.ps1`로 PM 라우팅 블록 재생성. **[PM] `project-manager` 1.4→1.5**(라우팅 대상 76종 / 묶음 15개).
- **[문서]** README(에이전트 76→77, 목차·번호 범위, 프론트엔드 표 행, 🎨 상세 블록 #77, 대칭 위임 3쌍 추가, 커맨드 표, sync 수치 81/55), AGENTS(제목·클러스터 표 프론트엔드 3→5·표 행·상세·대칭 위임), CLAUDE(에이전트 표 행), `design-agents.md`(디자인 계열 4종→5종) 동기화. **README 파일 트리의 누적 드리프트도 함께 정정** — 1.92 이후 추가된 13종(`project-manager`·`aws-reviewer`·`ai-code-auditor` 등)이 트리에서 누락돼 "(64개)"로 굳어 있던 것을 77개로 맞추고 `skills/` 항목에 `motion-reference`·`aws-deploy`를 반영했다.

## 1.104 (2026-07-22) — slide-deck-reviewer 호출 별칭 4종 추가

`/deck`·`/발표` 둘로는 떠오르는 대로 부르기 어렵다는 지적. 발표자료를 가리키는 말이 사람마다 다르므로(덱·PPT·슬라이드·프레젠테이션) 진입 경로를 넓혔다. 에이전트 정의는 **무변경**이고 커맨드 파일만 늘었다.

- **[커맨드] 4종 신설** — `/ppt`(가장 통용되는 호칭), `/슬라이드`, `/프레젠테이션`, `/프리젠테이션`. 마지막 둘은 **표기가 갈리는 외래어라 양쪽 모두** 뒀다(표준 표기는 "프레젠테이션"이지만 "프리젠테이션"으로 치는 경우가 흔하다 — 별칭의 목적은 규범 교육이 아니라 도달이다).
- 본문은 기존 `/deck`과 동일하며 1.103의 「틀만 보고 내용은 보지 않는다」 계약도 그대로 담았다. 6개 별칭 전부 같은 지시를 보낸다.
- `slide-deck-reviewer`는 이제 별칭 **6종**으로 이 저장소에서 `code-reviewer`(9종) 다음으로 진입 경로가 많다.
- **[문서]** README(sync 설명 79→80 슬래시 / 한글 별칭 51→54, 버전 표·상세 블록 헤더·커맨드 표), AGENTS(표 행) 동기화. 커맨드 파일 총 134개(영문 80 + 한글 54).

## 1.103 (2026-07-22) — slide-deck-reviewer 1.0→1.1: 「틀만 본다」 범위 확정

1.102 직후 실제 발표 덱으로 첫 실전 점검을 돌렸더니 **범위가 새어 나갔다.** 18건 중 4건이 내용 판정이었다 — 지표끼리 앞뒤가 안 맞는다는 지적, 발표 대본과 슬라이드 내용이 한 장씩 어긋난다는 대조, 같은 수치가 소스마다 다르다는 지적, 챕터 뱃지가 목차와 맞지 않는다는 지적. 개별로는 전부 맞는 말이지만 **프로젝트가 바뀌면 답이 바뀌는 것**이라 규약 감사자가 판정할 자리가 아니다. 사용자 지적("틀만 학습해. 내용은 프로젝트마다 전부 다르다")을 정의에 못박았다.

- **[핵심] 「다루지 않는 것 (내용)」 절 신설** — 6항목 표로 제외 범위와 담당을 고정: 수치의 사실성·상호 정합(`fact-checker`), 발표 대본↔슬라이드 내용 일치(작성자 판단 — 어느 쪽이 최신인지는 코드가 모른다), 챕터 구성·목차 순서·장 분할(`proposal-strategist`·`curriculum-designer`), 문구 설득력·분량(`copy-reviewer`·`brand-voice-guardian`), 차트 종류·축 범위(작성자), 슬라이드 필요성·순서(작성자). 지적하고 싶어지면 발견 목록에 넣지 말고 출력 맨 끝에 "내용 영역 — 담당 에이전트로 이관" 한 줄로만 남기게 했다.
- **[핵심] 경계선 판별법을 명문화** — "규약 문서(정본)에 적을 수 있으면 틀, 프로젝트가 바뀌면 답이 바뀌면 내용". 실전에서 갈렸던 세 쌍을 예시로 박았다: "페이지 번호가 배열 순서와 어긋난다"=틀 / "Chapter 8 뱃지가 목차와 다르다"=내용, "본문이 11px"=틀(정본이 하한을 정함) / "이 문장이 너무 길다"=내용. 판별 기준 없이 제외 목록만 두면 새 유형에서 다시 샌다.
- **[전제]** "틀만 본다. 내용은 보지 않는다"를 「일반 웹 화면과 판정 기준이 다르다」 앞에 배치 — **"이 값이 맞는가"가 아니라 "이 덱이 규약대로 굴러가는가"**만 묻는다.
- **[description]** 같은 계약을 라우팅 신호에도 반영(제외 4종 + 담당 + 경계선 판별법 요약).
- 점검 6축·출력 형식·심각도 척도는 **무변경**. 범위를 좁혔을 뿐 검사 항목을 뺀 것이 아니다.

## 1.102 (2026-07-22) — 신설: slide-deck-reviewer(#76) → 76종

`D:\presentation\`(발표자료를 프로젝트별로 모으는 상위 폴더)과 그 구조 규약 `INDEX.md`가 생기면서, **규약은 있는데 그것을 검사할 주체가 없는** 상태가 됐다. 발표 덱은 웹 화면처럼 생겼지만 판정 기준이 정반대라(반응형이 아니라 고정 캔버스 + 스케일, 스크롤이 아니라 잘림, 브라우저 거리가 아니라 빔프로젝터 투사) `ui-ux-reviewer`에 맡기면 오진한다. 전담 에이전트를 신설했다.

- **[신설] `slide-deck-reviewer` v1.0 (#76)** — 발표 슬라이드 덱의 구조 규약 감사. `opus`/`effort: high`/`Read, Grep, Glob`/`color: pink`.
  - **규약 정본을 에이전트가 직접 읽는다** — `D:\presentation\INDEX.md`를 `Read`로 열어 기준 삼고, 부재 시 내장 기준으로 돌면서 출력 첫 줄에 그 사실을 밝힌다. 절대경로를 정의에 두는 것은 `memory-recaller`·`self-reflector`·`knowledge-gardener`(`E:\claude_memory`)와 1.101의 `brand-voice-guardian`(`D:\auto_agent_content\voice.md`) 선례를 따른다(user-scoped 배포 전제). 정본은 사용자 자산이라 읽기만 한다.
  - **점검 6축**: ① 슬라이드 파일 ↔ 뷰어 `slides` 배열 ↔ import ↔ **하드코딩 페이지 번호** 동기화 — 중간 삽입 시 번호가 자동으로 안 밀리는데 빌드는 통과해 발표 당일에야 드러나는 가장 조용한 결함이다. 고아 슬라이드(작성했는데 배열에 없어 안 나오는 장)도 여기서 잡는다 ② 고정 캔버스 이탈(`.slide` 최상위 누락, 1440×810 밖 좌표, 스케일과 충돌하는 `vh`/`%`/미디어쿼리)과 **투사 가독성 최소 폰트**(본문 14px·카드 제목 16px) ③ 조작 UI 계약 — `data-nav` 누락이면 버튼을 누르는 순간 슬라이드가 넘어간다, 시스템 `alert` 금지, `use client` 과부족 ④ 에셋 참조 대 실제 파일 **전수 대조**(결손·미사용)와 디렉터리 혼동·비ASCII 파일명 ⑤ **템플릿 잔재** — 기존 덱을 복사해 시작하면 반드시 생기는 미교체 고유값(`name`·포트 충돌·`metadata`·PPTX 출력 파일명·로고·이전 프로젝트 팀 표기). "다른 프로젝트 이름이 발표 중에 떠 있는 사고"가 여기서 나온다 ⑥ `turbopack: {}` 누락과 **동적 import라 빌드는 통과하고 실행 시점에 죽는** 패키지의 의존성 미선언
  - **심각도 척도를 코드 냄새가 아니라 사고 크기로 정의** — 치명(슬라이드 누락·깨진 이미지·타 프로젝트 이름 노출·클릭 오작동) / 주의(안 읽힘·잘림·번호 어긋남) / 제안(규약 이탈이나 이번 발표엔 영향 없음). 리뷰어가 관객에게 보이는 결과를 기준으로 정렬하게 만드는 장치다.
  - **정적 한계를 명시** — 실제 넘침·투사 가독성·PPTX 충실도는 읽어서 단정할 수 없으므로 "확인 필요 + 확인 방법"으로 분리하고, "빽빽해 보인다" 같은 주관 판정을 금지했다(수치로 셀 수 있는 것만 단정).
- **[역방향 카브아웃] `ui-ux-reviewer` 1.6→1.7** — 신설 에이전트가 **라우팅 고아**가 되지 않도록(아무도 위임하지 않으면 라우터가 영영 안 고른다) description에 "발표 슬라이드 덱은 반응형·스크롤·WCAG 전제가 성립하지 않으므로 slide-deck-reviewer" 절을 추가하고, 자신을 "일반 웹 화면 전용"으로 좁혔다. 점검 항목·출력 형식은 무변경.
- **[커맨드] `/deck`·`/발표` 신설** — 79개 슬래시 명령(한글 별칭 51개).
- **[레지스트리]** `registry.json`에 `slide-deck-reviewer`(group 화면, note "발표 슬라이드 덱(고정 캔버스)") 추가 → `build-registry.ps1`로 PM 라우팅 블록 재생성.
- **[PM] `project-manager` 1.3→1.4** — 라우팅 블록에 `slide-deck-reviewer` 반영(`build-registry.ps1` 재생성). 라우팅 대상 75종 / 묶음 15개.
- **[문서]** README(에이전트 수 75→76, 목차·번호 연속 범위 1~76, 프론트엔드 표 행, 🎨 디자인 상세 블록 #76, 일방향 위임 표에 `ui-ux-reviewer → slide-deck-reviewer`, 커맨드 표, 파일 트리, sync 설명 수치), AGENTS(제목·클러스터 수·표 행), CLAUDE(에이전트 표 행) 동기화.

## 1.101 (2026-07-22) — 3폴더 연결: voice.md 정본 배선 + 세션·메모리 훅 6종 편입

1.99의 3폴더 교차검증을 다시 돌려, **"닫았다고 기록했지만 실제로는 절반만 닫힌" 이음새**를 마저 연결했다. 이번엔 문서 정합이 아니라 **배선**이다 — 경로를 아는 주체가 없으면 계약은 사문화된다.

- **[핵심] brand-voice-guardian 1.1→1.2 — 정본 경로를 에이전트가 알게 했다.** 1.99는 정본 위치를 `auto_agent_content\voice.md` **안에** 적어 이음새를 닫았다고 기록했지만, 그 위치를 아는 유일한 경로가 "그 파일을 이미 찾은 경우"였다. 에이전트 정의·`/voice` 커맨드 어디에도 경로가 없어, 실측상 D: 전체에 voice.md는 정본 1개뿐 → **어느 콘텐츠 프로젝트에서 `/voice`를 돌려도 100% "기준 부재"로 떨어졌고**, 폴백 4번이 `/fable`로 **두 번째 voice.md를 만들라고** 안내해 드리프트를 유도하고 있었다.
  - 기준 소스를 4단계 → **5단계**로 재편: ① 프로젝트 voice.md → ② **보이스 정본 `D:\auto_agent_content\voice.md`를 `Read`로 직접 읽음**(사용자 지정 경로가 있으면 그쪽 우선) → ③ 확정 예시글 → ④ 제공 예시 추론 → ⑤ 전부 부재. 절대경로를 정의에 두는 것은 `memory-recaller`·`self-reflector`·`knowledge-gardener`가 `E:\claude_memory`를 명시하는 기존 선례를 따른다(이 세트는 user-scoped 배포 전제).
  - **폴백 ⑤를 두 갈래로 분리** — "정본은 있는데 이 프로젝트에만 없음" → 새로 만들지 말고 **정본을 복사해 오라고 안내**, "정본 자체가 없음" → 그때만 `/fable`. 출력 요약에 **어느 파일을 기준으로 썼는지 경로까지** 명시하도록 변경(정본/사본 혼동 방지).
  - 정본은 라이브러리 밖 사용자 자산이라 **읽기만 하고 고치지 않는다**를 명시.
- **[커맨드] `/voice`** — 기준 소스 순서에 정본 경로 추가(에이전트 정의와 같은 계약).
- **[정본] `auto_agent_content\voice.md`** — 헤더의 "프로젝트에 복사해 두라 / 없으면 기준 부재" 문구를 실제 동작(직접 읽음)에 맞게 갱신하고, 사본을 두면 정본과 갈라질 수 있다는 경고와 §11 갱신 로그 1줄 추가. **보이스 규칙 자체는 무변경**(4·6·10축은 여전히 마케팅 존댓말 샘플 대기).
- **[훅] 세션·메모리 훅 6종을 저장소로 편입** — `memory-map`·`index-read-trigger`·`observe-capture`·`obsidian-session-log`·`obsidian-memory-mirror`·`precompact-snapshot`이 `~/.claude/hooks\`에만 있고 **D:·E: 어디에도 사본이 없었다**(검색 확인). `settings.json`이 6개를 전부 참조하는데 원본이 런타임뿐이라, C: 유실 시 자기개선 루프(observe-capture)·메모리맵·옵시디언 미러가 **복구 불가**로 사라지는 상태였다. 바이트 동일 복사로 `hooks/`에 편입해 `sync.ps1`(배포 대상 7종, 변경 없음)이 소유하게 했다. 시크릿 스캔 결과 키·토큰 없음(경로만 개인 값).
  - **배선은 편입하지 않았다** — 어느 이벤트에 어느 훅이 붙는지는 `~/.claude/settings.json`이고, 이 저장소가 관리하지 않는다는 사실을 CLAUDE.md·AGENTS.md에 명시.
- **[문서] CLAUDE.md에 「Adjacent folders」 절 신설** — 3폴더 관계를 표로 고정: content는 **실제 의존**(단방향 읽기), secagent는 **의도적 무연결**(SDK 직접 호출 독립 프로세스 — CI 게이팅·SARIF·라이브 DAST·exe 배포는 secagent, 편집 중 코드의 세션 내 리뷰는 서브에이전트). "연결이 없다"가 누락이 아니라 결정임을 적어, 다음 감사가 같은 지점을 재발견하지 않게 했다.
- **[secagent] `CLAUDE.md` 신설** — `examples/vulnerable_*`는 **의도적 취약 픽스처**이고 `.env`에 실키가 있어, 그 폴더에 `/sec`·`/aicode`를 돌리면 오탐이 쏟아지던 문제를 문서 계약으로 차단(픽스처를 "고치면" 도구의 검증 능력이 사라진다는 경고 포함). `secagent.spec`은 빌드 산출물이라 손대지 말고 `build_exe.ps1` 플래그를 고치라는 규칙, 격리 한계 문구를 완화하지 말라는 규칙 명시.
- **[문서 드리프트 정리]** README `sync.ps1` 설명의 에이전트 수 74→75, 한글 별칭 49→50 정정(1.100 신설 반영 누락). README 트리·AGENTS 위치 표의 `hooks/` 항목을 훅 7개 체제로 갱신. brand-voice-guardian 버전 표·요약 토큰·상세 블록 v1.2 동기화.
- **[미조치 — 사용자 확인 필요]** `auto_agent_secagent\.env`에 "리뷰 과정에서 노출됐으니 revoke하라"는 NEEDS-USER 주석이 남은 채 키가 그대로 있다. 파일만으로 교체 여부를 판별할 수 없어 **키 폐기·재발급은 사용자 몫**으로 남긴다(값은 어디에도 기록하지 않음).

## 1.100 (2026-07-21) — 신설: job-posting-scout(#75) → 75종

새 커리어 에이전트 1종 추가. 기존 `cover-letter-tailor`(고른 공고용 자소서 작성)의 **앞단** — "어디에 지원할지"를 국내 채용 사이트에서 찾아 정리한다. 둘은 "공고 탐색 ↔ 자소서 작성" 대칭 쌍.

- **[신설] job-posting-scout v1.0** (`/jobscout` `/채용공고`, opus·effort high·color cyan, tools Read/Grep/Glob/WebSearch/WebFetch) — 🧑‍💼 커리어 클러스터. 국내 채용 사이트(원티드·점핏·사람인·잡코리아·링크드인·리멤버·랠릿·게임잡)와 기업 공식 채용 페이지에서 사용자 프로필·전략에 맞는 **현재 지원 가능한** 정규직 우선 공고를 탐색·선별해 12열 지원 관리표로 정리. 대기업·대형 계열사 우선(부족하면 이유 먼저 + 적합도 높은 중견/중소/스타트업 보조후보), 이미 지원·마감 제외, 오늘 지원 가능만, 요구사항 60~70% 매칭이면 후보, 후순위 신호(석사·논문·계약직 단독 등) 반영.
  - **정직성이 존재 이유**: 기업명·공고·링크·마감일을 지어내지 않고 확인 가능한 것만, 로그인·차단으로 못 본 소스(원티드·링크드인·리멤버 등)는 "확인 불가"로 표시하고 직접 확인 URL 제공(추측 금지), 확인 시점 명시 + 지원 직전 재확인 안내(공고는 변동 큰 정보). 입력(직무·기술·도메인·경력·지역·지원완료목록)이 비면 지어내지 말고 되묻는다.
  - 출력: 소스 접근 요약 → 지원 관리표(후보 10건) → 공고별 적합 이유 → 앞세울 포트폴리오 → "오늘 바로 지원할 순서". 지원을 대신 실행하지 않고 탐색·선별·정리까지.
- **[카브아웃] cover-letter-tailor 1.1→1.2** — job-posting-scout로 가는 역방향 위임절 추가. "공고 탐색 ↔ 그 공고용 자소서" 대칭 쌍 신설(위임쌍 64→65).
- **[레지스트리]** `registry.json`에 job-posting-scout(교육·창작·커리어) 추가 → `build-registry.ps1`이 PM 라우팅 블록 재생성(라우팅 대상 73→74). **project-manager 1.2→1.3**.
- **[커맨드]** `/jobscout`·`/채용공고` 2종 신설.
- **[문서]** README·AGENTS 카운트 74→75, 버전 표·상세 블록·요약 라인 동기화, opus 69→70종. 콘텐츠 표의 커리어 #75는 # 오름차순 규칙대로 말미에 배치.
