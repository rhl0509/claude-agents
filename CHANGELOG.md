...
- **표 재구성** — 기존 38행 단일 표는 *추가된 순서*(1 code-reviewer … 38 debugger)라, 같은 일에 쓰는 에이전트가 표 곳곳에 흩어져 있었다(예: 보안 3종이 2·24·25행). 역할 기준 9개 클러스터의 소표로 나눴다: 🔍코드 품질·디버깅·테스트(5) / 🔒보안(3, 설계→코드→AI 순) / 🗄데이터·DB(3, 설계→튜닝→마이그레이션 순) / 🏗아키텍처·API·문서(4) / 🎨프론트엔드(3) / 🚀운영(3) / 🎮게임(7, 설계→코드→손맛→UI→성능→플레이테스트→빌드 순) / 📣콘텐츠(7, storyteller 포함) / 🧭메타·인프라(3).
- **번호 보존** — `#`은 상세 블록 번호와 대응하므로 **재번호하지 않고** 행 순서만 바꿨다(표 → 상세 블록 탐색이 계속 맞는다). 슬래시 칸에 1.65의 한글 별칭을 함께 표기.
