...
## 1.119 (2026-08-13) — statusline.ps1: 한글 세션 이름이 들어오면 상태줄이 통째로 비던 버그

`[Console]::In.ReadToEnd()`은 stdin을 **콘솔 코드페이지**(한국어 Windows = CP949)로 읽는다. 그런데 Claude Code가 주는 JSON은 UTF-8이고, 거기엔 자동 생성된 `session_name`이 들어 있다. 세션 이름이 한글이면 그 구간이 깨지면서 따옴표가 유실되고 → `ConvertFrom-Json`이 통째로 실패 → `$in`이 null → 폴더·모델·게이지가 전부 빈 채로 `[] □□□□□□□□□□ 0%`만 남는다. 영문 세션에서는 재현되지 않아 1.118 검증을 통과했다.

- **수정**: `[Console]::OpenStandardInput()`을 UTF-8 `StreamReader`로 직접 읽는다. 4행의 `OutputEncoding` 설정은 *출력*만 UTF-8로 바꿔줄 뿐 입력에는 영향이 없었다 — 입출력 양쪽을 다 잡아야 한다.
- **교훈**: PS 5.1 파이프라인에서 한글은 소스 파일 인코딩(1.118의 `■/□` 코드포인트 우회)뿐 아니라 **stdin 경로에서도** 깨진다. 이 레포의 다른 훅들도 stdin으로 JSON을 받으므로 같은 결함이 있는지는 별건으로 확인 필요.
...
- **정렬선 일관성(세로) 신설** — 가로 어긋남(래퍼별 좌우 패딩 중복)은 잡았지만, `.content`(수직 중앙) ↔ `.contentTop`(상단)을 섞어 쓴 두 장은 못 잡았다. **한 장씩 보면 안 보이고 연속 재생에서만 드러나는** 부류라 항목으로 못박았다. 실제로 표 슬라이드 2장만 아래가 150px 비어 넘길 때 제목 높이가 튀었다.
- **발표 장비 이식성 절(§7) 신설** — 이모지를 아이콘으로 쓰면 OS·폰트 버전마다 글리프와 폭이 달라 **발표장 노트북에서 다르게 렌더된다**. 인라인 SVG(`currentColor`) 대체를 처방으로 넣었다. 웹폰트 CDN 단일 의존(오프라인 시 폴백 폭 차이 → 줄 추가 → 잘림)은 에이전트가 원리에서 스스로 찾아낸 것이라, 재발견에 맡기지 말고 항목으로 고정했다.
- **아트 위 텍스트 스크림** — 기존 색 대비 항목은 단색 배경 전제였다. 배경 이미지·아트 워시를 깔면 **밝은 영역과 어두운 영역에 같은 색 텍스트가 걸쳐** 한쪽이 반드시 죽는다. 실제로 표지 하단 3단 스트립의 세 번째 항목이 수풀 위에 얹혀 안 읽혔다.
- **「정적 리뷰의 한계」에 실측 절차 명시** — 기존 문구는 "해당 슬라이드를 띄워 확인"에서 끝나 확인 필요가 그대로 남았다. `getBoundingClientRect()`를 캔버스 좌표로 환산(`k = rect.width/1440`)해 전 장을 순회하는 절차와, 텍스트 노드를 **직접** 가진 요소만 `fontSize`를 세는 단서(부모까지 세면 상속값 중복)를 적었다. 이 절차로 이번 덱의 "여유 68px, 폴백 시 위험" 계산이 넘침 0건으로 닫혔다. 에이전트는 실행 권한이 없으므로 **메인 세션이 돌릴 절차**로 제시하게 했다.
- 미사용 클래스에 잠복한 폰트 하한 위반도 보고 대상으로 명시(그 클래스를 나중에 쓰면 위반이 조용히 재발한다).
...
외부 도구 graphify(코드베이스를 `graph.json` 색인으로 만들어 파일 대신 그걸 조회하는 CLI+스킬)를 도입하고 실사용 프로젝트 1건(FastAPI+Next, 코드 110파일)에 적용한 결과의 반영. **에이전트를 만들지 않고 커맨드 2종만 추가했다.**

- **에이전트를 만들지 않은 이유**: graphify는 이미 자체 스킬(`~/.claude/skills/graphify/SKILL.md`, `/graphify`)로 설치된다. 서브에이전트로 감싸서 얻는 건 "SKILL.md 703줄이 메인 컨텍스트에 안 들어온다" 정도인데 스킬은 필요할 때만 로드되므로 실익이 얇다. 더 큰 이유는 **인터페이스를 아직 모른다는 것** — 실사용 1회차에 실제로 쓴 건 `explain`/`affected`와 즉석 파이썬 파싱이었고 `graphify query`는 한 번도 안 썼다. 무엇을 자동화할지 모르는 채로 껍데기를 짜게 된다. 반복이 확인된 절차 2개만 커맨드로 고정했다.
- **`/지도`** — 생성·갱신·조회를 한 커맨드에서 분기(질문이면 조회, 경로면 생성, 비었으면 상황에 따라). 기본 `--code-only`(AST만 → **LLM 토큰 0**), 문서 포함은 토큰을 쓰므로 사용자에게 먼저 묻게 했다.
- **`/지도대조`** — "누가 X를 부르는가"가 아니라 **"불러야 하는데 안 부르는 곳"** 을 전수 대조. 실사용에서 이 방식으로 API 계층의 상태 변경 함수 45개 중 1개의 감사 기록 누락을 파일을 열지 않고 찾았다. 모집단 정의가 유일한 판단 지점이라 **정규식을 판정 근거로 그대로 노출**하게 했고(숨기면 "전수"가 거짓이 된다), 누락/의도를 단정하지 말고 양쪽 근거를 대게 했다.
- **실사용에서 나온 함정 2개를 커맨드에 못박았다**: ① **단일 프로젝트에 `merge-graphs` 금지** — 하위 폴더별로 뽑아 병합하면 노드에 `repo` 태그가 박혀 이후 갱신이 `ValueError: nodes span multiple repos`로 죽고 `--force` 전체 재추출로만 풀린다. ② **커뮤니티 라벨은 `cluster-only`·`update` 재실행 시 날아간다**(번호 재배정) — 라벨 유지가 목적이면 재클러스터링 없이 `graphify export html`만 돌린다. 라벨 붙이는 5단계 절차도 함께 적었다.
- **한글 이름이 정본**: 두 커맨드는 영어 커맨드의 별칭이 아니다. `/지도`의 영어 대응은 이 레포가 아니라 graphify가 자체 설치하는 `/graphify`이고, `/지도대조`는 영어 쌍이 없다. 기존 한글 별칭 표(1.65)와 성격이 달라 README에 별도 문단으로 구분했다.
- ⚠️ **`graphify install`이 전역 `~/.claude/CLAUDE.md` 끝에 3줄을 무단 append한다**(스킬 안내 + "다른 무엇보다 먼저" 우선순위 지시). 사용자 지시로 삭제했고 스킬은 그 3줄 없이도 동작한다. **재설치·업데이트 시 재발 가능** — 도구 상세는 메모리 토픽 `project_claude_token_tools`.

## 1.113 (2026-07-29) — image-prompt-engineer 1.2: 일관성 레이어(레퍼런스 시트) 신설

출처는 사용자가 공유한 외부 노션 가이드(레퍼런스 보드 프롬프트 6종). **에이전트를 만들지 않고 기존 1종에 층을 얹었다.**

- **발견한 공백**: `image-prompt-engineer` 1.1은 5계층으로 이미지 **한 장**을 정밀하게 만드는 데까지였고, **같은 대상이 여러 장에 반복될 때의 일관성**은 통째로 비어 있었다. 프롬프트를 더 정교하게 써서 풀리는 문제가 아니라 별도 층이라 안티패턴 표·슬롯 패턴 어디에도 들어갈 자리가 없었다.
- **신설하지 않은 이유**: 별도 에이전트(`reference-board-designer`)로 떼면 도메인·산출물(프롬프트 텍스트)·권리 경계가 기존 1종과 전부 겹쳐 경계 중복 + 라우팅 모호("이미지 프롬프트 만들어줘"가 두 곳으로)가 된다. skill 분리(`design-reference` 선례)도 검토했으나 **단일 에이전트 전용 + 2KB대**라 1.109의 판단 기준(공유 대상·분량)에 못 미쳐 본문 절로 넣었다.
- **원본을 그대로 옮기지 않았다**: 남의 저작물이라 6종 프롬프트 전문 복제 대신 **축(axis) 설계 원칙으로 재구성**했고, 원본의 특정 시각값(용지색·라벨 색)은 "브랜드 톤 치환 슬롯"으로 일반화했다. 유형 목록이 아니라 원칙이라 가이드에 없는 유형(의상·인테리어·타이포)도 파생된다고 명시.
- **반영**: 「일관성 레이어」 절 신설 — 발동 조건(2회 이상 반복 시에만, 단발엔 과잉), 대상 유형별 변량 축 표 6행, 축 6개 실용 한계, 필수 구성 4가지(전 패널 불변 고정문·프로필 텍스트 박스·hex 팔레트·시트 시각 규격), 5계층 접속 규칙(**조명·기술은 패널별이 아니라 시트 전체에 한 번만**), 2단계 워크플로와 플랫폼 네이티브 수단(`--cref`/`--sref`·IP-Adapter·LoRA)과의 선택 기준. 절차 1·3·4단계와 출력 형식(`## 일관성 전략` 블록) 동반 갱신.
- **기존 조항과의 충돌을 봉합**: 원본은 "인물 사진 한 장 넣으세요"지만 이 에이전트에는 **실존 인물 얼굴 생성 금지** 조항이 이미 있다. 시트는 얼굴을 여러 각도·표정으로 **증식**시켜 단발보다 리스크가 크므로 — 본인 또는 동의받은 모델 사진일 때만 진행, 아니면 가상 인물을 먼저 생성해 그 결과물로 시트를 만드는 경로를 대안으로 — 를 「권리 접점」에 못박고 「권리·윤리 경계」 절에도 상호참조 불릿을 넣었다. 두 절이 따로 읽히면 모순되기 때문. 시트 안 라벨에 대해서는 기존 「이미지 속 텍스트」(한글 미렌더) 규칙이 그대로 적용됨도 명시.
- **`⚠️검증필요` 유지**: 원본은 일관성이 "딱 고정된다"고 하지만 2단계 참조의 유지력은 모델·버전 편차가 크다. 패널 수와 해상도의 관계도 같은 표기.
- **경계 확장 1건**: 6종 중 **샷 보드(12컷 콘티)만 경계에 걸린다** — 확인 결과 `video-optimizer.md`에 콘티·스토리보드·샷 관련 내용이 0건이라 "무엇을 어떤 순서로 보여줄지"의 기획 담당이 없다. 다만 이 보드는 관용 화각 사다리(와이드→익스트림 클로즈)로 고정된 상수라 기획 판단이 사실상 없어 이 에이전트가 그대로 다룬다. description에 썸네일과 동일한 패턴의 위임절(기획은 `video-optimizer`·`content-repurposer`, 시트 이미지는 이쪽)만 추가했고 **`video-optimizer`는 손대지 않았다**(양방향 링크가 이미 존재해 라우팅 고아 아님). 영상 서사 기획 자체의 공백은 별도 사안으로 남긴다.

## 1.112 (2026-07-28) — 덱 모션 공백 닫기 + 클라우드 설계 공백 기록

1.111 감사에서 "의논 대기"로 남긴 3건의 처리. **2건 실행, 1건 기각.**

- **[실행] 발표 덱 모션의 주인 지정 — `motion-reviewer` 1.0 → 1.1.** 종전엔 `motion-reviewer`가 덱을 통째로 `slide-deck-reviewer`에 넘겼는데, 그쪽은 "틀만 본다"는 원칙이라 모션을 다루지 않는다(모션·애니메이션·트랜지션 언급 0회 확인) — **위임은 걸려 있고 받는 쪽이 거부해 주인이 없는 상태**였다. 대칭 카운트로는 안 잡히는 부류다. 구조 규약(배열 동기화·캔버스 넘침·`data-nav`·에셋)은 그대로 넘기고 **모션 값 판정만** 가져왔다. 반대 방향(slide-deck에 모션 절 추가)은 그쪽의 "틀만 본다" 원칙을 깨서 택하지 않았다. **덱 전제에 맞지 않는 축은 명시적으로 뺐다** — 빔프로젝터 투사·키보드 조작이라 빈도 게이트(하루 100회)와 호버 포인터 게이트는 성립하지 않는다. `prefers-reduced-motion`은 유지. 본문 「다루지 않는 것」 표도 함께 정정(description만 고치면 본문과 모순됐다).
- **[실행] 클라우드 인프라 *설계* 공백을 `CLAUDE.md`에 기록.** IaC가 아직 없는 단계의 "이 서비스를 AWS에 어떤 구성으로 올릴까"(서비스 선택·VPC/서브넷·ECS vs Lambda·비용 구조)는 `aws-reviewer`(있는 IaC를 읽는다)·`system-architect`(앱 내부 구조)·`aws-deploy` 스킬(IaC 존재 전제) 어디에도 없다. **에이전트를 만들지 않았다** — 대칭 답은 `aws-architect`(C/.NET/Java/Swift의 리뷰어↔아키텍트 쌍과 동형)지만 승격 게이트가 "필요가 **반복**될 때"를 요구하고 지금은 `expense_tracker` 한 건뿐이다. 그 배포 후 재검토하도록 조건까지 적었다. **다른 알려진 공백(파이썬 백엔드 perf·Java/Swift perf·Unity 테스트 실행·Android/Kotlin)과 달리 기록조차 없던 것이 문제였다** — 공백 자체보다 다음에 또 조사하게 되는 비용이 크다.
- **[기각] `slide-deck-reviewer` description 압축.** 77종 중 최장이지만 **오라우팅이 실제로 일어났다는 증거가 없다.** 판별 신호("고정 캔버스 발표 덱"·"1440×810")는 앞쪽에 있고, 압축하면 `PPTX`·`turbopack`·`data-nav` 같은 트리거 어휘가 사라져 재현율이 떨어진다. 측정 없는 처방이라 1.108과 같은 이유로 보류 — 실제 라우팅 실패 사례가 나오면 그 사례를 근거로 고친다.
...
- **`CLAUDE.md` 에이전트 표에 `job-posting-scout` 행 누락**(76행) → 추가해 77행. 1.100/1.106에서 신설·개정될 때 이 표에만 반영되지 않았다. README·AGENTS 표에는 있었다.
- **README 표 버전 드리프트 2건** → `game-feel-reviewer` 1.3→**1.4**, `project-manager` 1.4→**1.5**(둘 다 실제 프론트매터가 정본, 표가 낡았던 것). README 상단 요약의 `game-feel-reviewer` v1.3도 함께 정정 — 요약은 `project-manager` v1.5로 이미 맞아 있어 **같은 파일 안에서 요약과 표가 서로 달랐다.**
- **린터가 "에이전트 76종"이라 찍는 것은 정상**이다. `registry.json`의 `agents`가 76인 이유는 `project-manager`가 `entryPoints`로 분리된 **의도된 고아**(모든 것 위의 조율 계층)이기 때문 — 76 + 1 = 77. 이 숫자를 볼 때마다 재조사하지 않도록 여기 적어둔다.
- 검증: README 표 번호 1~77 연속·중복 없음, 77종 전부 등재, 표 버전 ↔ 프론트매터 전수 대조 불일치 0. lint 77파일 ERROR 0/WARN 0.
- **알려진 구멍**: 린터는 README/AGENTS에 "행이 존재하는지"만 보고 **표의 버전 값이 프론트매터와 맞는지는 검사하지 않으며, `CLAUDE.md` 표는 검사 대상이 아니다.** 이번 3건이 전부 그 틈으로 샜다. 자동 검사 확장은 별도 작업으로 남긴다.

## 1.109 (2026-07-28) — design-reference에 차트·데이터 시각화 절 신설

`nextlevelbuilder/ui-ux-pro-max-skill` **재검토**. 1.87에서 이미 채굴한 곳이라 이번 작업은 "그때 안 가져온 것 중 지금 값어치가 생긴 게 있는가"였다.

- **발견한 공백**: 원본 `charts.csv`(대시보드 25종)에 대응하는 층이 우리에게 **아예 없다.** `차트`/`chart` grep 히트는 전부 오탐이었다(level-designer의 "페이싱 차트", devops의 "Helm 차트", curriculum의 "시각화 지점"). `ui-ux-reviewer`·`design-system-architect` 점검 항목에도 없다.
- **승격 게이트 통과**: 추측이 아니라 실제 작업에서 반복된다 — `airport_skyteam`의 `app/sightings/stats.tsx`(노선별 발견율 막대·요일별 분포), `stock_tracker`·`erp`·`expense_tracker` 대시보드. 하니스 `dataviz` 빌트인이 있지만 **서브에이전트는 못 쓴다**(리뷰어 tools가 `Read, Grep, Glob`뿐이라 Skill 로드 경로 없음) — 빌트인이 있어도 이 공백은 남는다.
- **반영**: `skills/design-reference/SKILL.md`에 5번 절 신설(기존 체크리스트는 6번으로 밀림). 차트 종류는 데이터 관계가 정한다(비교→막대·추이→선·구성→누적막대·분포→히스토그램·상관→산점도), 막대 축은 0에서 시작(잘린 축 = 과장), 색은 인코딩이지 장식이 아니다(범주형/순차형/발산형·8범주 상한), 색만으로 구분 금지, 데이터 잉크(격자·3D 제거·직접 라벨), `tabular-nums`·단위 접두 혼용 금지, 0건/1건/로딩/에러 상태, 대체 텍스트·표 대안. 체크리스트에도 한 줄 추가.
- **새 에이전트를 만들지 않았다.** 이 스킬이 이미 두 디자인 에이전트에 프리로드되므로 파일 1개 수정으로 끝나고 에이전트 77종 버전은 건드리지 않는다 — 1.87이 쓴 방식 그대로다.
- **여전히 안 가져온 것**: CSV 데이터셋(192팔레트·74폰트·161룰·25차트) 원본은 그대로 미채택(리뷰어는 전수 검색이 필요 없고 데이터는 원본이 유지보수한다). `search.py`(BM25 검색·생성기)는 우리 에이전트에 실행 경로가 없어 채택 불가. `stacks.csv`(22 스택)는 언어별 아키텍트와 중복, `guidelines.csv`(98 UX)는 `ui-ux-reviewer`가 이미 커버. **5번 절은 원본 `charts.csv`를 옮긴 것이 아니라 통상적 시각화 관행 기준의 자체 집필**이며, 스킬 상단 출처 주석에 그 사실을 명시했다.

## 1.108 (2026-07-28) — code-reviewer 1.18: Top 3는 재인용이 아니라 색인

`JuliusBrussee/caveman`(출력 토큰 압축 스킬 + 에이전트 3종) 검토에서 **유일하게 근거가 선 항목 하나만** 반영. 나머지는 전부 기각했고 그 판단을 남긴다.

- **채택**: `code-reviewer` 1.17 → 1.18. 출력 형식에 "Top 3는 색인" 조항 추가 — 항목 제목 + `파일:줄` + "왜 먼저인가" 한 줄만 적고, 근거·영향·수정 방향을 본문에서 재진술하지 않는다. 근거는 같은 날 돌린 리뷰 9판에서 Top 3가 발견 내용을 근거까지 통째로 다시 쓴 것(편당 대략 150~300단어). **줄일 대상을 "중복된 설명"으로 못박고 증거·위치·불확실성 표기는 명시적으로 보호**했다 — 분량 압박이 발견 억제로 새는 것이 장황함보다 비싸기 때문.
- **기각 — 에이전트 3종**: `cavecrew-reviewer`는 우리 `code-reviewer`의 열화 중복(출력이 `path:line: 심각도: 문제. 수정.` 한 줄뿐, 요약·근거·영향·부류 전체 점검 소실). `cavecrew-builder`는 코드를 직접 쓰므로 이 세트의 핵심 계약(읽기 전용 + `PreToolUse` 가드 훅)과 구조적 충돌. `cavecrew-investigator`(haiku 코드 위치 탐색)는 우리에게 담당이 없는 **진짜 공백**이지만 하니스 `Explore` 빌트인이 같은 일을 하고 승격 게이트("실제 필요가 반복될 때만")에 걸려 보류.
- **기각 — caveman 스킬 본체**: "hedging을 버려라"가 이 라이브러리의 *"확신 없으면 확인 필요/추정 표시"* 규범과 정면 충돌. 출력 형식 계약(심각도 블록·Top 3·`파일:줄`)도 압축 대상이 된다. 그쪽 `HONEST-NUMBERS.md`도 65%는 출력만이고 턴당 입력 1~1.5k 증가, 세션 총 14~21%, 짧은 응답에선 마이너스임을 밝히고 있다.
- **기각 — 보고 밀도 규범 일반화**: `agent-conventions`에 "칭찬·서론·툴 호출 서술 금지" 류를 넣는 안을 검토했으나 철회. 리뷰 9판을 다시 보니 길이의 원인이 패딩이 아니라 발견 수였고(서두·인젝션 보고는 규범이 요구하는 항목), **측정 없이 처방하는 것**이라 레포 자체 규칙("측정과 단정을 분리한다")에 걸린다. 근거가 선 Top 3 항목만 좁혀서 반영했다.

## 1.107 (2026-07-28) — 코드 리뷰어 6종: **범위 규율**(과잉설계·범위 이탈) 렌즈 신설

출처는 `multica-ai/andrej-karpathy-skills`(Karpathy의 LLM 코딩 실패 관찰 4개 규범). 사용자가 "에이전트로 추가"를 요청했으나 **이건 에이전트가 아니라 행동 규범**(CLAUDE.md 1장 + 동일 내용 SKILL.md)이라 그대로 넣지 않고, 기존 규칙과 대조해 **공백만 흡수**했다.

- **폐기 2건**: 원칙 1("불확실하면 멈추고 물어라")은 사용자 전역 규칙 §5 "질문보다 초안 먼저"와 정면 충돌 → "가정을 침묵으로 처리하지 않는다" 한 줄만 살림. 원칙 3("인접 코드 손대지 마라")은 전역 §4 "기계적 일관성은 즉시 맞춘다"와 부분 충돌 → 예외를 명시(무관한 개선은 금지, 대칭 정렬은 의무).
- **`code-reviewer` 1.16 → 1.17**: `## 범위 규율 (과잉설계 · 범위 이탈)` 섹션 신설. ① 요청에 없는 것(쓰이지 않을 확장점·설정·유연성, 호출자가 하나뿐인 추상화, 일어날 수 없는 경우의 예외 처리) ② 분량 대비 일("시니어 개발자가 과하다고 할 코드인가") ③ 범위 이탈(변경 줄의 요청 역추적 — 무관한 리팩터·주석 정리·포맷 변경·인접 코드 "개선") ④ 고아 정리의 범위(내 변경이 만든 것만 제거, 기존 죽은 코드를 함께 지운 것도 이탈). 심각도는 기본 Should fix, **요청 범위 밖에서 동작이 바뀌었으면 Must fix**.
- **형제 리뷰어 4종 v1.0 → v1.1, `unity-code-reviewer` 1.5 → 1.6**: 같은 섹션을 복붙하지 **않았다**. 5종 모두 `## 공통`에 "일반 품질 폴백은 code-reviewer가 맡는다"가 이미 있어 **의도된 비대칭**이므로, 정본은 code-reviewer 한 곳에 두고 나머지는 기존 위임 문장과 같은 형식의 **한 줄**만 추가했다. 언어별 예시는 각자 관용구로(C 매크로 래퍼 / .NET·Java 인터페이스·팩토리 / Swift 프로토콜 / Unity 매니저 싱글턴·이벤트 버스·ScriptableObject 래퍼).
- **안 건드린 것**: `refactor-strategist`(이미 "동작 보존 최우선 + 분리 항목"으로 커버 — 추가는 중복), `agent-conventions` 스킬(보고 규범이지 탐지 렌즈가 아니라 storyteller·copy-reviewer에도 걸림), 6종 `description`(기존 "코드 리뷰" 트리거로 도달 가능 — 부풀리면 그 자체가 범위 이탈). 새 에이전트 신설은 76종 대비 경계 중복이라 기각.
- **실코드 검증 9판**: 커밋 6판(diff+요청 있음)에서 강하게 발동 — `.mlua`·Flask 템플릿 등 **스택 밖에서도** 걸렸고(메이플마피아 `MAX_WRITE_RETRIES 3→5` 부수 변경, crack의 "특수문자 받게" 요청에 8종 화이트리스트 = 과대 이행), 포트 재배분 커밋을 이탈로 **오탐하지 않았다**(기계적 일관성 예외 정상 작동). 기존 파일 3판(diff·요청 없음)은 과잉설계 반쪽만 약하게 발동 — unity가 `[CreateAssetMenu]`인데 `.asset` 0개인 ScriptableObject 래퍼를 정확히 적중. `java`·`dotnet`은 D드라이브에 대상 코드가 없어 **미검증**으로 남긴다.

## 1.106 (2026-07-22) — job-posting-scout 1.1: 정직성 규율을 서술에서 **기계적 게이트**로

실패가 먼저 있었다. 평일 09시 클라우드 실행(노션 적재분)이 **SK Careers R261170을 1순위로 올렸는데, 접수기간이 2026.05.22~05.31로 이미 52일 지난 공고**였다(실제 회사도 "SK그룹"이 아니라 인크로스). 그날 실행은 **모든 소스가 403**이라 원문을 하나도 못 읽었고, 표의 마감일 칸은 전부 "확인 필요"였다. 그런데도 1순위에 올린 근거가 이것뿐이었다 — *"'하반기' 명칭이 현재(7월) 시점과 일치해 접수 중일 가능성이 가장 높은 유일한 대기업 후보"*.

같은 페이지에서 SK텔레콤·현대오토에버·카카오는 마감을 **확인해서** 제외해놓고, 정작 **확인하지 못한 건을 맨 위에** 올린 것이다. 1.100 정의는 이미 "확인 가능한 공고만 표에 올린다"고 적고 있었으므로 — **규율은 있었고 지켜지지 않았다.** 서술형 규범이 "대기업 후보를 채워야 한다"는 압력 앞에서 무너진 사례라, 판정을 문장이 아니라 통과/탈락 게이트로 바꿨다.

- **[게이트 A] 마감일 미확인 = 표 밖으로.** 원문에서 접수기간·마감일을 직접 읽지 못했으면 적합도와 무관하게 지원 관리표에 넣지 않고, 우선순위·적합도를 매기지 않는 **`확인 못 한 후보(지원 판단 불가)`** 표로 내린다(회사·직무·링크·못 본 이유·사용자가 확인할 것). **"확인 필요"는 셀 값이 아니라 표 밖으로 나가는 사유**라는 문장으로 못박았다 — 종전엔 같은 단어가 표 안에 머무를 수 있어서, 미확인이 곧 추천이 되는 경로가 열려 있었다.
- **[게이트 B] 접수 시작 60일 경과는 마감 간주.** 마감일을 몰라도 **시작일만으로** 걸러낸다. 국내 신입·수시 공고의 통상 접수기간이 2~4주라, 60일이면 상시채용이 원문에 명시되지 않는 한 닫힌 것으로 본다. 이번 SK 건은 마감일을 모르더라도 **"5월 공고"라는 사실 하나로** 걸러졌어야 했다. 제외 시 "시작일 YYYY-MM-DD, 60일 경과" 근거를 남긴다.
- **[게이트 C] "가능성이 높다"는 근거가 아니다.** 공고명·시즌·회사 규모·업계 관행에서 끌어낸 추론을 표 진입 근거로 금지하고, **대기업 후보 부족이 게이트를 무르게 만드는 주된 압력**임을 정의에 명시해 "모자라면 빈칸으로 두고 사유를 적는다"를 규칙으로 세웠다. 출력 형식의 "후보 10건"도 **"목표 10건이되 게이트 통과분이 적으면 적은 채로"**로 고쳤다 — 정원이 곧 채움 압력이었다.
- **[수집 실패 보고] 소스 과반 차단이면 표를 만들지 않는다.** 탐색 대상의 과반이 403·로그인·JS 렌더 실패면 관리표 대신 **"오늘 수집 실패"**를 결론으로 낸다(시도 소스별 응답, 스니펫으로만 본 이름은 **후보가 아니라 단서**로, 직접 확인 URL, 재시도 권고). 같은 날 세션 실행에서는 잡코리아가 정상 응답한 반면 09시 클라우드 실행에서는 403이었으므로, **차단은 그날·그 환경의 사실이지 사이트의 항구적 성질이 아니라는 점**도 함께 적었다(다음 실행에서 재시도).
- 선별 규칙에 5·7번(60일 게이트 / 마감일 확인 건만)을 추가하고, 본문 말미를 **"후보를 채우는 것보다 비우는 것이 낫다 — 만료 공고 하나가 사용자의 하루를 버리게 만든다"**로 닫았다.
- **[문서]** README(상단 버전 요약 v1.0→v1.1, 커리어 표 행, 상세 블록에 게이트·수집 실패 항목 2개 신설), AGENTS(표 행·상세 블록) 동기화. 에이전트 수·커맨드 수 변화 없음(77종 유지).
