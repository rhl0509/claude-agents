...
## 1.65 (2026-07-14) — 한글 슬래시 별칭 실험: `/디버그`

- **`commands/디버그.md` 신설** — `/debug`와 내용이 같은 한글 별칭(에이전트는 그대로 `debugger` 1종, 커맨드 파일만 2개). Claude Code가 슬래시 명령 이름에 **비ASCII를 허용하는지는 공식 문서에 명시가 없어 검증용**이다. 다음 세션에서 `/디버그`가 목록에 뜨면 성공, 안 뜨면 파일만 지우면 원복(에이전트·다른 커맨드에 영향 없음). 결과에 따라 다른 에이전트로 확대할지 결정한다.
- **주의** — 한국어 **자연어 호출**은 이 별칭과 무관하게 이미 동작한다(38종 description이 전부 한국어라 라우터가 한국어 문장으로 매치). 별칭은 슬래시 명령 표기 편의일 뿐이다.
- **문서** — README 슬래시 명령 표에 `/디버그` 행 추가. **배포** — `sync.ps1`(커맨드 39개).
